...
Follow these. Emmett drops the `sqlite` path segment, since `emmett-sqlite` is already the SQLite package; everything else matches.
...
Specs move with their driver, so a folder holds the driver and everything that tests it. The driver name leaves the filename, because the folder already carries it. Only relative imports change; spec bodies do not.
...
Mirrors Pongo file for file. Six files change or appear:
...
src/
  ...
  packages/emmett-sqlite/
    ...
    src/testing/durableObjectTestWorker.ts        # new
```
...
"exclude": [
  "./src/storage/durableObject/**/*.int.spec.ts",
  "./src/storage/durableObject/**/*.e2e.spec.ts",
  "./src/testing/durableObjectTestWorker.ts"
]
```
...
```jsonc
{
  ...
  "include": [
    ...
    "packages/emmett-sqlite/src/storage/durableObject/**/*.int.spec.ts",
    "packages/emmett-sqlite/src/storage/durableObject/**/*.e2e.spec.ts",
    "packages/emmett-sqlite/src/testing/durableObjectTestWorker.ts",
  ],
  "exclude": [],
}
```

`worker-configuration.d.ts` is generated by `wrangler types`, not written by hand, and gives the `env.TEST_OBJECT` binding its type.
...
```jsonc
{
  ...
  "main": "src/testing/durableObjectTestWorker.ts",
...
`packages/emmett-sqlite/src/testing/durableObjectTestWorker.ts` — deliberately empty, as in Dumbo. The object exists only so `runInDurableObject` has something to hand real `ctx.storage` out of; the event store is constructed inside each spec, not here:
...
"types:cloudflare": "wrangler types packages/emmett-sqlite/worker-configuration.d.ts --config packages/emmett-sqlite/wrangler.durable-object.jsonc",
"types:cloudflare:check": "wrangler types packages/emmett-sqlite/worker-configuration.d.ts --config packages/emmett-sqlite/wrangler.durable-object.jsonc --check"
```
...
**Proof this step works:** one trivial Durable Object spec that opens a pool through the driver and runs `SELECT 1`, confirming the runner boots and `runInDurableObject` yields usable storage.
