# Emmett event sourcing on Cloudflare D1

A shopping cart API for Cloudflare Workers. Each cart is an event stream stored in D1 by the [Emmett](https://event-driven-io.github.io/emmett/) SQLite event store, with [Pongo](https://github.com/event-driven-io/Pongo) read models for queries and [Hono](https://hono.dev/) routing requests.
...
- keeping each cart as a stream of events in D1 and deciding changes with plain functions,
- updating Pongo read models in the same transaction as the events, so reads never lag behind writes,
- keeping concurrent changes to the same cart safe, using the stream version and the `ETag` and `If-Match` headers,
- running the same cart code on another store: the [Durable Objects sample](../durable-objects/README.md) uses the same event-sourced model and differs only in how storage is wired.
...
Emmett stores what happened to the cart as events instead. [`businessLogic.ts`](./src/shoppingCarts/businessLogic.ts) gets a command and the current cart state, and returns the new events. `handleCommand` reads the cart's stream, runs that function, and appends the events only if nobody appended to the stream first. If somebody did, the append fails, and the API answers `412 Precondition Failed`. Emmett's SQLite event store runs the same way on D1, on Durable Object SQLite and on a local SQLite file.

Queries read Pongo documents. Two inline projections turn the events into documents in the same D1 database, inside the transaction that appends the events. The events and the read models commit or roll back together.

Hono routes the request. Emmett fills the gap between routing and storage: it reads `If-Match`, writes `ETag`s, turns a concurrency conflict into `412` and a rejected business rule into `409`, and formats errors as problem details. That keeps try/catch out of the routes. Emmett also provides the test API, so [`api.int.spec.ts`](./src/shoppingCarts/api.int.spec.ts) states the events that already happened, a request, and the response it expects.

## How the cart is modelled

Each cart has its own event stream. The cart ID is the stream name, `shopping_cart-{clientId}:{n}`, where `n` is the client's cart number (1, 2, 3, ...). The same ID is the `_id` in the response body and the last segment of the cart's URL, for example `/clients/c1/shopping-carts/shopping_cart-c1%3A3`.

The stream holds these events, defined in [`shoppingCart.ts`](./src/shoppingCarts/shoppingCart.ts):

| Event                                | When                                         |
| ------------------------------------ | -------------------------------------------- |
| `ShoppingCartOpened`                 | The client's first product opens a new cart. |
| `ProductItemAddedToShoppingCart`     | A product is added, with its unit price.     |
| `ProductItemRemovedFromShoppingCart` | A quantity of a product is removed.          |
| `ShoppingCartConfirmed`              | The cart is confirmed.                       |
| `ShoppingCartCancelled`              | The cart is cancelled.                       |

Every event carries the client ID in its metadata. The first product appends `ShoppingCartOpened` and `ProductItemAddedToShoppingCart` together, in one append to the new stream. Confirming a confirmed cart or cancelling a cancelled cart appends nothing, so retrying is safe and the `ETag` does not change.

Two inline projections, in [`projections`](./src/shoppingCarts/projections), keep the read models:

- `shoppingCarts` has one document per cart stream: the client, the status, the product lines with the unit price captured when each product was added, the item count, the total amount, and the stream position of the last event. The `GET` endpoints read it.
- `clientShoppingCarts` has one document per client: the last cart number, the last cart ID, and whether that cart is `Opened` or `Closed`. Only adding a product through `/current` reads it.

[`pongo.config.ts`](./src/pongo.config.ts) declares both collections with their document types. The projections take their collection names from it, and the read-side Pongo client in [`pongo.ts`](./src/pongo.ts) uses it for typed queries. It also declares a non-unique index on `clientId` and `status` for the current-cart query. The projections pass these typed collections to the event store, so its migration creates the collections and this index.

Adding a product through `/current` reads the client's `clientShoppingCarts` document. If its cart is open, the product goes to that cart. Otherwise, the product opens cart `n + 1`, and the append expects the stream not to exist yet. When two requests open the same new cart at the same time, one append fails, and its retry adds the product to the cart that the other request opened. So a client never has two open carts, and no unique index is needed.

Confirming or cancelling a cart does not delete it: the cart stays readable under the same URL, and the next product added through `/current` opens a new one.
...
Responses carry the cart's stream version as a weak `ETag`, such as `W/"3"`: the position of the last event in the cart's stream. Commands return the version after their append. `GET` endpoints return the stream position kept in the `shoppingCarts` document. Send that value back in `If-Match` when you change a cart through its permanent URL. You get `412 Precondition Failed` if the cart changed in the meantime, `404 Not Found` if it does not exist, and `409 Conflict` if its current state forbids the change, such as confirming an empty cart. Errors are problem details ([RFC 9457](https://www.rfc-editor.org/rfc/rfc9457)).
...
[`src/eventStore.ts`](./src/eventStore.ts) creates the event store for the Worker, with the Cloudflare `d1EventStoreDriver`, the `DB` binding from [`wrangler.jsonc`](./wrangler.jsonc) read through `env` from `cloudflare:workers`, and both inline projections. [`src/pongo.ts`](./src/pongo.ts) creates the read-side Pongo client on the same binding, with `throwOnOperationFailures`, so a failed operation throws instead of returning a flag.

The event store owns the whole schema: the event tables, the migration ledger, and the read model collections. Local development uses `autoMigration: "CreateOrUpdate"`, so the first cart operation initializes a fresh local database. Production uses `autoMigration: "None"`, so cart requests never change the schema. The read-side Pongo client always uses `autoMigration: "None"`.

`npm run deploy` migrates the production schema right after deploying. It deploys the Worker with `wrangler deploy`, stores a fresh random token with `wrangler secret put MIGRATION_TOKEN`, and calls the bearer-protected `POST /_system/migrations` endpoint with curl. The endpoint runs `eventStore.schema.migrate()`, which creates the event tables, the migration ledger, and the read model collections. Every deploy generates a new token, so earlier tokens stop working. The endpoint sits outside the shopping-cart API, and the event store owns the whole schema, so the sample keeps no separate Wrangler SQL migrations.
...
2. The script creates the tables. It calls the deployed Worker's `POST /_system/migrations` endpoint, which runs the event store migration and creates the event tables, the migration ledger, and the read model collections. Every later deploy runs it again and applies only what's missing.
...
```text
Deployed to https://emmett-shopping-cart-d1.<your-workers-subdomain>.workers.dev
```
...
```shell
url=https://emmett-shopping-cart-d1.<your-workers-subdomain>.workers.dev
...
You'll see the cart with `product-1`, a quantity of `2`, and its `ETag`, `W/"2"`.
...
Wrangler deploys the Worker named `emmett-shopping-cart-d1` into the selected account. With `workers_dev: true`, its URL is normally:

```text
https://emmett-shopping-cart-d1.<your-workers-subdomain>.workers.dev
```
