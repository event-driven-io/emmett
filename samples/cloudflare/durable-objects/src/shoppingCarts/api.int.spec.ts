...
import { shoppingCarts } from '../pongo.config';
import { shoppingCartApi } from './api';
import { getUnitPrice } from './pricing';
import type { ShoppingCartEvent } from './shoppingCart';
import { getShoppingCartEventStore } from './shoppingCartDurableObject';

describe('Durable Object shopping-cart API integration', () => {
  it('indexes shopping carts by client and status', () =>
    runInDurableObject(
      env.SHOPPING_CARTS.getByName(crypto.randomUUID()),
      async (_instance, state) => {
        await getShoppingCartEventStore(state.storage).schema.migrate();

        const indexes = state.storage.sql
          .exec<{ name: string }>(
            `SELECT name FROM sqlite_master WHERE type = 'index' AND tbl_name = ?`,
            shoppingCarts.tableName,
          )
          .toArray()
          .map(({ name }) => name);

        expect(indexes).toContain('shopping_carts_client_status');
      },
    ));
      ...
      .then([
        async (response) => {
          await expectResponse(201, {
            headers: { etag: 'W/"2"' },
            body: {
              _id: `shopping_cart-${cart.clientId}:1`,
  ...
  it('allows a client to revisit a known shopping cart', () => {
    ...
    const cart = shoppingCart(crypto.randomUUID(), now);
  ...
  it('reduces the product quantity in a shopping cart', () => {
    ...
    return given(
      ...
      ...removedProductFromShoppingCart(cart, 'product-1', 1),
    )
      .when(getShoppingCart(cart))
      .then([
        async (response) =>
...
const existingEvents = (
  ...
  const shoppingCartId = `shopping_cart-${cart.clientId}:1`;

  cart.location = `/clients/${cart.clientId}/shopping-carts/${shoppingCartId}`;
  cart.version += events.length;
  cart.previousETag = `W/"${cart.version - 1}"`;
  cart.eTag = `W/"${cart.version}"`;

  return existingStream<ShoppingCartEvent>(shoppingCartId, events);
};
...
const openedShoppingCart = (
  ...
  return [
    existingEvents(cart, [
      ...
      ...productItems.map(({ productId, quantity }): ShoppingCartEvent => ({
...
const removedProductFromShoppingCart = (
  cart: TestShoppingCart,
  productId: string,
  quantity: number,
) => {
    ...
    existingEvents(cart, [
      {
        ...
        data: {
          shoppingCartId: shoppingCartId(cart),
          productItem: {
            productId,
            quantity,
            unitPrice: unitPrices[productId],
...
const confirmedShoppingCart = (
  ...
  const opened = openedShoppingCart(cart, ...productItems);
  const { clientId } = cart;
    ...
    existingEvents(cart, [
      {
        ...
        data: { shoppingCartId: shoppingCartId(cart), confirmedAt: new Date() },
        metadata: { clientId },
      },
    ]),
  ];
    ...
    existingEvents(cart, [
      {
        ...
        data: { shoppingCartId: shoppingCartId(cart), cancelledAt: new Date() },
        metadata: { clientId },
      },
    ]),
  ];
...
const givenApi = (now = new Date()) =>
  ApiSpecification.for<ShoppingCartEvent, EventStore>({
    getEventStore: clientsEventStore,
    getApplication: () =>
      getApplication({
        apis: [
          shoppingCartApi({
...
const clientsEventStore = (): EventStore => {
  const inClientDurableObject = <Result>(
    streamName: string,
    handle: (eventStore: EventStore) => Promise<Result>,
  ) =>
    runInDurableObject(
      env.SHOPPING_CARTS.getByName(clientIdOf(streamName)),
      (_instance, state) => handle(getShoppingCartEventStore(state.storage)),
    );

  return {
    aggregateStream: (streamName, options) =>
      inClientDurableObject(streamName, (eventStore) =>
        eventStore.aggregateStream(streamName, options),
      ),
    ...
    streamExists: (streamName) =>
      inClientDurableObject(streamName, (eventStore) =>
        eventStore.streamExists(streamName),
      ),
  };
};

const clientIdOf = (shoppingCartId: string) =>
  shoppingCartId.slice(
    'shopping_cart-'.length,
    shoppingCartId.lastIndexOf(':'),
  );
