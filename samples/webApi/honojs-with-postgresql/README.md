...
# Emmett - Sample showing event-sourced WebApi with Hono and PostgreSQL
...
```
{service_name="honojs-with-postgresql"} |= "<trace-id>"
```
...
**HTTP** — HTTP request rate and response latency percentiles from the auto-instrumented Node.js HTTP layer
...
When using Podman through a Docker-compatible socket, disable the Testcontainers
Ryuk sidecar if it cannot observe container startup:

```bash
TESTCONTAINERS_RYUK_DISABLED=true npm run test
```
