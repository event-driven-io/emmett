...
Failure in an event-sourced system starts in one small place, a decision, and works its way outward: through the command handler, the asynchronous handlers that react to events, and finally the web API. This guide follows that path. It shows the choice you make in the business logic, returning a failure event or throwing an error, then how each layer above treats what you chose.
...
The command handler runs inside a request, so an error thrown along the way unwinds safely to the web layer. Asynchronous event handlers, projections, reactors, and workflow output handlers, run on their own, with no request to unwind to. An error thrown there propagates up and stops the processor, so the handler makes no further progress and later events go unprocessed. Two options keep it under control.
...
At the edge of the synchronous path, a thrown error becomes an HTTP response. Behind `emmett-expressjs`, `getApplication` installs middleware that turns it into a [Problem Details](https://www.rfc-editor.org/rfc/rfc9457.html) body. Problem Details is the IETF standard (RFC 9457) for describing an HTTP error in the response body: a small, agreed set of fields (`type`, `title`, `status`, `detail`) so clients get one consistent, machine-readable error shape instead of each API inventing its own. Emmett gives you that shape for free. An `IllegalStateError` becomes a 403:
...
If a read model stops updating and later events go unhandled, an asynchronous handler most likely threw. A thrown error there is not caught back into a request; it stops the processor. Return a failure event, or a `SKIP` or `STOP` result, instead of throwing, as [Never Throw in Asynchronous Event Handlers](#no-throw-async) shows.
