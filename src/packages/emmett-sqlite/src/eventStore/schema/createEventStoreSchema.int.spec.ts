...
const tableExists = async (db: SQLiteConnection, tableName: string): Promise<boolean> => {
  const result = await db.querySingle<TableExists>(`SELECT name FROM sqlite_master WHERE type='table' AND name='${tableName}';`);
