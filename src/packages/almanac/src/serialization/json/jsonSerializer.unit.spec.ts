...
describe('JSON Serializer', () => {
  describe('JSONReplacers', () => {
    ...
    describe('error', () => {
      it('converts Error to plain object', () => {
        ...
        const result = JSONReplacers.error('key', err) as {
          type: string;
          message: string;
      ...
      it('uses constructor name as type for subclasses', () => {
        class CustomError extends Error {}
        const err = new CustomError('custom');
        const result = JSONReplacers.error('key', err) as { type: string };

        assert.strictEqual(result.type, 'CustomError');
      });

      it('passes non-Error values through unchanged', () => {
        assert.strictEqual(JSONReplacers.error('key', 'hello'), 'hello');
        assert.strictEqual(JSONReplacers.error('key', 42), 42);
        assert.deepStrictEqual(JSONReplacers.error('key', { a: 1 }), { a: 1 });
      });
    });
  });
  ...
  describe('jsonSerializer', () => {
    ...
    it('serializes with format: compact produces no indentation', () => {
      const serializer = jsonSerializer();
      const data = { a: 1, b: [2, 3] };
      const result = serializer.serialize(data, { format: 'compact' });

      assert.strictEqual(result, '{"a":1,"b":[2,3]}');
    });

    it('serializes with format: pretty produces 2-space indentation', () => {
      const serializer = jsonSerializer();
      const data = { a: 1, b: [2, 3] };
      const result = serializer.serialize(data, { format: 'pretty' });

      assert.strictEqual(result, JSON.stringify(data, null, 2));
    });

    it('serializes with safe: true returns fallback for circular reference', () => {
      const serializer = jsonSerializer();
      type CircularRef = { a: number; self?: CircularRef };
      const circular: CircularRef = { a: 1 };
      circular.self = circular;
      const result = serializer.serialize(circular, { safe: true });
      const parsed = JSON.parse(result) as { __serializationError: string };

      assert.ok('__serializationError' in parsed);
    });

    it('throws on circular reference without safe', () => {
      const serializer = jsonSerializer();
      type CircularRef = { a: number; self?: CircularRef };
      const circular: CircularRef = { a: 1 };
      circular.self = circular;

      assert.throws(() => serializer.serialize(circular));
    });

    it('serializes Error objects by default', () => {
      const serializer = jsonSerializer();
      const result = serializer.serialize({ err: new Error('boom') });
      const parsed = JSON.parse(result) as {
        err: { type: string; message: string };
      };

      assert.strictEqual(parsed.err.type, 'Error');
      assert.strictEqual(parsed.err.message, 'boom');
    });

    it('serializes Error as empty object when skipErrorSerialization is true', () => {
      const serializer = jsonSerializer();
      const result = serializer.serialize(
        { err: new Error('boom') },
        { skipErrorSerialization: true },
      );
      const parsed = JSON.parse(result) as { err: Record<string, unknown> };

      assert.deepStrictEqual(parsed.err, {});
    });
  });
