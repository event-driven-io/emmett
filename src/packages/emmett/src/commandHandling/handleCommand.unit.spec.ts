...
void describe('Command Handler', () => {
  ...
  void describe('commandType resolution', () => {
    const namedHandler = function addProductItemCommand(
      _state: ShoppingCart,
    ): ShoppingCartEvent {
      return {
        type: 'ProductItemAdded',
        data: {
          productItem: { productId: 'p', quantity: 1, price: 1 },
        },
      };
    };

    void it('falls back to options.commandType when handleOptions does not carry one', async () => {
      const tracer = collectingTracer();
      const meter = collectingMeter();

      const handler = CommandHandler<ShoppingCart, ShoppingCartEvent>({
        evolve,
        initialState,
        name: 'label-name',
        commandType: 'option-type',
        observability: { tracer, meter },
      });

      await handler(getInMemoryEventStore(), randomUUID(), namedHandler);

      const span = tracer.spans.find((s) => s.name === 'command.handle');
      expect(span).toBeDefined();
      expect(span!.attributes[EmmettAttributes.command.type]).toBe(
        'option-type',
      );
    });

    void it('falls back to options.name when options.commandType is absent', async () => {
      const tracer = collectingTracer();
      const meter = collectingMeter();
    ...
    void it('falls back to the handle function name for a single handler', async () => {
      const tracer = collectingTracer();
      const meter = collectingMeter();
    ...
    void it('records handler names as an array when handle is an array of named functions', async () => {
      const tracer = collectingTracer();
      const meter = collectingMeter();
      ...
      await handler(getInMemoryEventStore(), randomUUID(), [
        namedHandler,
        function confirmCommand(): ShoppingCartEvent {
          return { type: 'DiscountApplied', data: { percent: 0 } };
        },
      ]);
    ...
    void it('leaves emmett.command.type undefined when no source is available (anonymous arrow)', async () => {
      const tracer = collectingTracer();
      const meter = collectingMeter();
      ...
      const span = tracer.spans.find((s) => s.name === 'command.handle');
      expect(span).toBeDefined();
      expect(span!.attributes[EmmettAttributes.command.type]).toBeUndefined();
    });
  });

  void describe('correlation forwarding', () => {
    void it('forwards correlationId and causationId from handleOptions.observability to span attributes and event metadata', async () => {
      const tracer = collectingTracer();
      const meter = collectingMeter();
      const eventStore = getInMemoryEventStore();
      const appendSpy = vi.spyOn(eventStore, 'appendToStream');
      ...
      const span = tracer.spans.find((s) => s.name === 'command.handle');
      expect(span).toBeDefined();
      expect(span!.attributes[MessagingAttributes.message.correlationId]).toBe(
        'flow-x',
      );
      expect(span!.attributes[MessagingAttributes.message.causationId]).toBe(
        'cmd-x',
      );
    ...
    void it('reuses the same generated correlationId on the span attribute and the appended event metadata', async () => {
      const tracer = collectingTracer();
      const meter = collectingMeter();
      const eventStore = getInMemoryEventStore();
      const appendSpy = vi.spyOn(eventStore, 'appendToStream');
      ...
      const spanCorrelationId =
        span!.attributes[MessagingAttributes.message.correlationId];
      const appendedCorrelationId = appendSpy.mock.calls[0]![2]?.correlationId;

      expect(spanCorrelationId).toBeTypeOf('string');
      expect(appendedCorrelationId).toBe(spanCorrelationId);
    });
  });
});
