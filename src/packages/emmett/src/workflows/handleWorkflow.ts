...
export type WorkflowHandlerOptions<
  Input extends AnyEvent | AnyCommand,
  State,
  Output extends AnyEvent | AnyCommand,
  ...
  async <Store extends EventStore>(
    ...
    const sourceMetadata =
      'metadata' in message && message.metadata
        ? (message.metadata as Partial<MessageMetadataType>)
        : undefined;
    ...
    const result = await collector.startScope(
      ...
      async (scope) =>
        asyncRetry(
          () =>
            withSession<Store, WorkflowHandlerResult<Output, Store>>(
              store,
              async ({ eventStore }) => {
                const {
                  workflow: {
                ...
                const hasWorkflowPrefix = messageType.startsWith(
                  `${workflowName}:`,
                );

                // Separated inbox mode: store-only path (no prefix = external input)
                if (
                  options.separateInputInboxFromProcessing &&
                  !hasWorkflowPrefix
                ) {
                ...
                const wrappedInitialState =
                  createWrappedInitialState(initialState);
                ...
                collector.recordStateRebuild(
                  scope,
                  Number(currentStreamVersion),
                );
