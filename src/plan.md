...
- Allow `SQLTableReference.from` ergonomics that accept an omitted schema and internally use Dumbo's default-schema sentinel, or document that consumers should pass `DefaultDatabaseSchemaName`. Emmett's local `tableReference` helper exists mostly to hide that sentinel conversion. SQLite sharpens this: it has no default database schema name at all, so `DefaultDatabaseSchemaName` is the only way to express an unprefixed name, and both Dumbo and Pongo fall back to it. Emmett now hides the same conversion twice, in the PostgreSQL `tableReference` helper (`packages/emmett-postgresql/src/eventStore/schema/typing.ts:48`) and in the identical SQLite one (`packages/emmett-sqlite/src/eventStore/schema/typing.ts:47`).
...
- `collection.schema.migrate(options)` inherits only `migrationTable` from the client. `dryRun`, `ignoreMigrationHashMismatch`, `migrationTimeoutMs` and `session` reach a collection migration through the call argument or not at all, and dropping the argument fails silently. Emmett passes `context.migrationOptions` at every projection init call site instead. The same argument is also the only way to accept a changed collection migration, because Pongo records `table:pongo_collection:<schema>:<collection>:create` with no `ignoreHashMismatch` flag, unlike Emmett's own current migration.
- Pongo cannot build a client from an ambient connection without a driver-specific import. `connectionString` is missing from `PongoClientOptions<AnyPongoDriver>`, and passing `pool` instead loses the database name and lets Pongo check out a connection outside the projection transaction. Emmett's PostgreSQL Pongo call site therefore imports `pgDriver` directly, although both dialects now carry `driverType` in the projection context. This is the construction-side companion of the borrowed-pool item in `## Out of scope`.
- `pongoDriverRegistry.tryResolve` returns `null` for an unknown driver type, and `pongoClient` then fails on that null inside projector init instead of reporting which driver type could not be resolved. An Emmett context missing `driverType` cost a debugging pass because of it.
...
- `dryRun` depends on Dumbo owning the transaction. `runSQLMigrations` implements it by rolling back the transaction it opens, so a caller supplying its own `execute` runs the migration for real. Emmett supplies `execute: tx.execute` so schema hooks and migrations share one transaction, and therefore rolls that outer transaction back itself and returns `{ success: false, result }` on a dry run. A dry-run mode independent of transaction ownership would remove that workaround.
- `SQLCreateSchema` renders an empty string on SQLite and `runSQLMigrations` drops the empty migration with `rendersNothing`, so one dialect-free code path produces a real step on PostgreSQL and nothing on SQLite, with no signal either way. Emmett emits no create-schema statement on SQLite at all and pins that with a test. Documenting the no-op, or reporting the dropped step in the result, would make one shared path safe to write.
